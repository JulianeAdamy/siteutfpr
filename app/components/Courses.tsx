export default function Courses() {
    return (
        <main className="grid gap-12 md:grid-cols-2 lg:grid-cols-3">
            <div className="md:col-span-2 lg:col-span-3 mt-4 text-black text-left text-2xl">
                <h2 id="cursos">
                    Latest <span>Drops</span>
                </h2>
            </div>
            <div className="bg-[#FFFFFF] rounded-md pb-2 border border-gray-300">
  <a href="/curso_html.html">
    <img className="hover:scale-105 rounded-lg" src="https://github.com/JulianeAdamy/siteutfpr/assets/86632678/3be4b90a-0437-4370-9a98-36f20362b08e" alt="" />
    <div className="text-black text-left space-x-4 space-y-4">
    
    <h3>       </h3>
    <p className="text-black-500">ipad pro 05 standar... $ 22</p>
      <p className="text-gray-500">f11bc5b1 Photoshop</p>
    </div>
  </a>
</div>
            <div className="bg-[#FFFFFF] rounded-md pb-2 border border-gray-300">
                <a href="/curso_css.html">
                    <img className="hover:scale-105 rounded-lg" src="https://github.com/JulianeAdamy/siteutfpr/assets/86632678/62b56b62-bcd2-4512-9d4c-e649f0bc9138" alt="" />
                    <div className="text-black text-left space-x-4 space-y-4">
    
    <h3>       </h3>
    <p className="text-black-500">DahsFolio... $ 75</p>
      <p className="text-gray-500">2bef6a36 Framer</p>
                    </div>
                </a>
            </div>
            <div className="bg-[#FFFFFF] rounded-md pb-2 border border-gray-300">
                <a href="/curso_js.html">
                    <img className="hover:scale-105 rounded-lg" src="https://github.com/JulianeAdamy/siteutfpr/assets/86632678/0bed05e5-2a3d-4835-9f28-d83f02896d6f" alt="" />
                    <div className="text-black text-left space-x-4 space-y-4">
    
    <h3>       </h3>
    <p className="text-black-500">Distortion Collecti... $ 19</p>
      <p className="text-gray-500">c4b537ab Tallwind</p>
                    </div>
                </a>
            </div>
            <div className="md:col-span-2 lg:col-span-3 mt-4 text-black text-center text-2xl">
                <h2 id="cursos">
                    Mockups 
                </h2>
            </div>
            <div className="bg-[#FFFFFF] rounded-md pb-2 border border-gray-300">
                <a href="/curso_html.html">
                    <img className="hover:scale-105 rounded-lg" src="https://github.com/JulianeAdamy/siteutfpr/assets/86632678/72607c62-a0b0-403b-b60a-a7949a5d52b9" alt="" />
                    <div className="text-black text-left space-x-4 space-y-4">
    
    <h3>       </h3>
    <p className="text-black-500">CyberFolio... $ 35</p>
      <p className="text-gray-500">aee4b3c5 Framer</p>
                    </div>
                </a>
            </div>
            <div className="bg-[#FFFFFF] rounded-md pb-2 border border-gray-300">
                <a href="/curso_html.html">
                    <img className="hover:scale-105 rounded-lg" src="https://github.com/JulianeAdamy/siteutfpr/assets/86632678/da82fdc4-580b-421f-a332-46af76b976c6" alt="" />
                    <div className="text-black text-left space-x-4 space-y-4">
    
    <h3>       </h3>
    <p className="text-black-500">Future Folio... $ 49</p>
      <p className="text-gray-500">76ce1a25 Framer</p>
                    </div>
                </a>
            </div>
            <div className="bg-[#FFFFFF] rounded-md pb-2 border border-gray-300">
                <a href="/curso_html.html">
                    <img className="hover:scale-105 rounded-lg" src="https://github.com/JulianeAdamy/siteutfpr/assets/86632678/05feca88-5283-415e-aac6-5ba20b209c76" alt="" />
                    <div className="text-black text-left space-x-4 space-y-4">
    
    <h3>       </h3>
    <p className="text-black-500">Anomaly... $ Free</p>
      <p className="text-gray-500">25162b26 Framer</p>
                    </div>
                </a>
            </div>
            <div className="md:col-span-2 lg:col-span-3 mt-72 text-black text-center text-2xl">
                <h2 id="cursos">
                    Tempates
                </h2>
            </div>
            <div className="bg-[#FFFFFF] rounded-md pb-2 border border-gray-300">
                <a href="/curso_html.html">
                    <img className="hover:scale-105 rounded-lg" src="https://github.com/JulianeAdamy/siteutfpr/assets/86632678/3be4b90a-0437-4370-9a98-36f20362b08e" alt="" />
                    <div className="text-black text-left space-x-4 space-y-4">
    
    <h3>       </h3>
    <p className="text-black-500">MacBook Pro 08 Stan... $ 30</p>
      <p className="text-gray-500">3edf655a lllustrator</p>
                    </div>
                </a>
            </div>
            <div className="bg-[#FFFFFF] rounded-md pb-2 border border-gray-300">
                <a href="/curso_css.html">
                    <img className="hover:scale-105 rounded-lg" src="https://github.com/JulianeAdamy/siteutfpr/assets/86632678/62b56b62-bcd2-4512-9d4c-e649f0bc9138" alt="" />
                    <div className="text-black text-left space-x-4 space-y-4">
    
    <h3>       </h3>
    <p className="text-black-500">Iphone 14 Pro Stand... $ 19</p>
      <p className="text-gray-500">db787245 Photoshop</p>
                    </div>
                </a>
            </div>
            <div className="bg-[#FFFFFF] rounded-md pb-2 border border-gray-300">
                <a href="/curso_js.html">
                    <img className="hover:scale-105 rounded-lg" src="https://github.com/JulianeAdamy/siteutfpr/assets/86632678/0bed05e5-2a3d-4835-9f28-d83f02896d6f" alt="" />
                    <div className="text-black text-left space-x-4 space-y-4">
    
    <h3>       </h3>
    <p className="text-black-500">Wall Sign 01 Standa... $ 19</p>
      <p className="text-gray-500">0b2ea682 Framer</p>
                    </div>
                </a>
            </div>
            <div className="md:col-span-2 lg:col-span-3 mt-4 text-black text-center text-2xl">
                <h2 id="cursos">
                    Graphics
                </h2>
            </div>
            <div className="bg-[#FFFFFF] rounded-md pb-2 border border-gray-300">
                <a href="/curso_html.html">
                    <img className="hover:scale-105 rounded-lg" src="https://github.com/JulianeAdamy/siteutfpr/assets/86632678/72607c62-a0b0-403b-b60a-a7949a5d52b9" alt="" />
                    <div className="text-black text-left space-x-4 space-y-4">
    
    <h3>       </h3>
    <p className="text-black-500">Distortion Collecti... $ 19</p>
      <p className="text-gray-500">aee4b3c5 lllustrator</p>
                    </div>
                </a>
            </div>
            <div className="bg-[#FFFFFF] rounded-md pb-2 border border-gray-300">
                <a href="/curso_html.html">
                    <img className="hover:scale-105 rounded-lg" src="https://github.com/JulianeAdamy/siteutfpr/assets/86632678/da82fdc4-580b-421f-a332-46af76b976c6" alt="" />
                    <div className="text-black text-left space-x-4 space-y-4">
    
    <h3>       </h3>
    <p className="text-black-500">Transform Collection... $ 19</p>
      <p className="text-gray-500">76ce1a25 Photoshop</p>
                    </div>
                </a>
            </div>
            <div className="bg-[#FFFFFF] rounded-md pb-2 border border-gray-300">
                <a href="/curso_html.html">
                    <img className="hover:scale-105 rounded-lg" src="https://github.com/JulianeAdamy/siteutfpr/assets/86632678/05feca88-5283-415e-aac6-5ba20b209c76" alt="" />
                    <div className="text-black text-left space-x-4 space-y-4">
    
    <h3>       </h3>
    <p className="text-black-500">Undertone 03 Gradie... $ 11</p>
      <p className="text-gray-500">25162b26 Framer</p>
                    </div>
                </a>

                </div>
                
                <div className="md:col-span-2 lg:col-span-3 mt-36 text-black text-center text-2xl">
                <hr style={{ border: '2px solid black mt-16 ', width: '100%', margin: '0' }} />
                <h2 id="cursos">
                    From The <span>Meg</span>
                </h2>
            </div>
                <div className="bg-[#FFFFFF] rounded-md pb-2 border border-gray-300">
                <a href="/curso_html.html">
                    <img className="hover:scale-105 rounded-lg" src="https://github.com/JulianeAdamy/siteutfpr/assets/86632678/72607c62-a0b0-403b-b60a-a7949a5d52b9" alt="" />
                    <div className="text-black text-left space-x-4 space-y-4">
    
    <h3>       </h3>
    <p className="text-black-500">Distortion Collecti... $ 19</p>
      <p className="text-gray-500">aee4b3c5 lllustrator</p>
                    </div>
                </a>
            </div>
            <div className="bg-[#FFFFFF] rounded-md pb-2 border border-gray-300">
                <a href="/curso_html.html">
                    <img className="hover:scale-105 rounded-lg" src="https://github.com/JulianeAdamy/siteutfpr/assets/86632678/da82fdc4-580b-421f-a332-46af76b976c6" alt="" />
                    <div className="text-black text-left space-x-4 space-y-4">
    
    <h3>       </h3>
    <p className="text-black-500">Transform Collection... $ 19</p>
      <p className="text-gray-500">76ce1a25 Photoshop</p>
                    </div>
                </a>
            </div>
            <div className="bg-[#FFFFFF] rounded-md pb-2 border border-gray-300">
                <a href="/curso_html.html">
                    <img className="hover:scale-105 rounded-lg" src="https://github.com/JulianeAdamy/siteutfpr/assets/86632678/05feca88-5283-415e-aac6-5ba20b209c76" alt="" />
                    <div className="text-black text-left space-x-4 space-y-4">
    
    <h3>       </h3>
    <p className="text-black-500">Undertone 03 Gradie... $ 11</p>
      <p className="text-gray-500">25162b26 Framer</p>
                    </div>
                </a>
                </div>
        </main>

    )
}